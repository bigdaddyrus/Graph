...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Road road = _map.getLabel(segment.get(0), segment.get(1));
        String roadName = road.toString();
        ...
        for (first = 0; first < segment.size() - 1; first++) {
            last = first + 1;
            next = last + 1;
            if (next == segment.size()) {
                String loc =
                    _map.getLabel(segment.get(segment.size() - 1)).toString();
                System.out.printf("%d. Take %s %s for %.1f miles to %s.%n",
                    seq, roadName, dir, len, loc);
                seq++;
                break;
            }
            Road nextroad =
                _map.getLabel(segment.get(last), segment.get(next));
            String nroadName = nextroad.toString();
            String ndir = nextroad.direction().fullName();
            double nlen = nextroad.length();
            if (!nroadName.equals(roadName)
                     || !ndir.equals(dir)) {
                System.out.printf("%d. Take %s %s for %.1f miles.%n",
                    seq, roadName, dir, len);
                len = nlen;
                roadName = nroadName;
                dir = ndir;
                seq++;
