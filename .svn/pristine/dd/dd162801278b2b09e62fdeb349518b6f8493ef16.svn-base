...
class Maker {
    ...
    void readFileAges(String fileInfoName) {
        ...
        try {
            Scanner inp =
                new Scanner(new BufferedReader(new FileReader(fileInfoName)));
            ...
            error("Could not find file: %s", fileInfoName);
        }
    }
    ...
    class MakeTraversal extends DepthFirstTraversal {
        ...
        protected boolean postVisit(int v0) {
            ...
            if (!_targets.containsKey(target)) {
                if (!_ages.containsKey(target)) {
                    throw new Error("Error: Couldn't find target");
                }
            } else {
                r.rebuild();
