...
class Rule {
    ...
    void addDependency(Rule dependent) {
        _depends.add(_vertex, _depends.add(dependent));
    }
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty()
            && !_commands.isEmpty()) {
            throw new IllegalStateException("Error: Bad Commands");
        } else {
    ...
    private void checkFinishedDependencies() {
        for (int e: _depends.successors(_vertex)) {
            Rule r = _depends.getLabel(e);
            if (r.isUnfinished()) {
                r.rebuild();
            }
        }
    ...
    private boolean outOfDate() {
        if (_commands.isEmpty()) {
            if (_time == null) {
                throw new IllegalStateException("Error: No traget.");
            } else {
                return false;
            }
        } else if (_time == null) {
            return true;
        } else if (_depends.successor(_vertex, 0) == 0) {
            return false;
        } else {
            return _time < _maker.getCurrentTime();
        }
    }
    ...
    void rebuild() {
        try {
            ...
            throw new IllegalStateException("Error: Circular loop");
        }
        if (outOfDate()) {
            if (_commands.isEmpty()) {
